//
// Example code for article: https://lostmoa.com/blog/iOSKeyboardShortcutsInSwiftUI/
//
    ...
    override var keyCommands: [UIKeyCommand]? {
        ...
        if cityProvider.addedCities.count > 1,
            ...
            if selectedIndex > 0 {
                result.append(
                    UIKeyCommand(
                        action: #selector(selectPreviousCity), input: UIKeyCommand.inputUpArrow,
                        modifierFlags: [.shift, .command], discoverabilityTitle: "Select Previous City"
                    )
                )
            }
            ...
            if selectedIndex < cityProvider.addedCities.count - 1 {
                result.append(
                    UIKeyCommand(
                        action: #selector(selectNextCity), input: UIKeyCommand.inputDownArrow,
                        modifierFlags: [.shift, .command], discoverabilityTitle: "Select Next City"
                    )
                )
            }
        }
        ...
        if cityProvider.selectedCity != nil {
            result.append(
                UIKeyCommand(
                    ...
                    modifierFlags: [.command], discoverabilityTitle: "Delete City"
                )
            )
        }
...
extension UIKeyCommand {
    convenience init(action: Selector, input: String, modifierFlags: UIKeyModifierFlags, discoverabilityTitle: String) {
        self.init(
            title: "", image: nil, action: action, input: input, modifierFlags: modifierFlags,
            propertyList: nil, alternates: [], discoverabilityTitle: discoverabilityTitle, attributes: [], state: .on
        )
    }
}
